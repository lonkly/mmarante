<script>
    // Optional: Add any dynamic logic here if needed in the future
</script>

<div class="max-w-2xl mx-auto py-8 px-4 text-center bg-white shadow-lg rounded-lg">
    <h1 class="text-2xl font-bold mb-4 text-gray-800">Консультации онлайн</h1>
    <p class="text-lg font-medium mb-6 text-gray-600">
        <strong>Стоимость 55 минут:</strong> 80$ / 80€
    </p>
    <a href="/contacts" target="_blank" class="inline-block">
        <button
                type="button"
                class="bg-[#DDC8C4] hover:bg-[#C5A9A1] text-white font-semibold py-3 px-6 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105"
        >
            Записаться
        </button>
    </a>
</div>